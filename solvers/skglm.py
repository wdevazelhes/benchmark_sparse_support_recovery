    ...
    def run(self, grid_value):
        ...
        for alpha in self.alphaGrid:
            ...
            if self.estimator == "ksnn":
                ...
                solver = solver_class(penalty=L0_5(alpha), solver=AndersonCD(max_iter=self.max_iter, tol=1e-7, verbose=0, fit_intercept=False))
            elif self.estimator == "l23":
                solver = solver_class(penalty=L2_3(alpha), solver=AndersonCD(max_iter=self.max_iter, tol=1e-7, verbose=0, fit_intercept=False))
            else:
                solver = solver_class(
